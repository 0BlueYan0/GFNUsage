...
> 遊玩時段。
>
> **macOS 只在一台 Mac 上跑過。** 登入流程正是為了讓沒裝 GFN 客戶端的 Mac
> 也能自己登入而做的，但開發機是 Windows。登入、選單列項目、面板在一台
> Mac 上試過。
...
- ✅ **顯示在工作列上** —— Windows 上剩餘或已使用的時數和進度條直接放在
  工作列上，左邊或右邊。在設定裡打開
- ✅ **結轉與加購時數** —— 併入本期總額一併顯示
- ✅ **配速門檻** —— 算出「到現在為止該用多少」，超前就變紅
- ✅ **超支預測** —— 會在哪天用完、超出多少
- ✅ **本期走勢圖** —— 配額到現在怎麼用掉的，照最近七天的速度月底會走到哪
- ✅ **玩完就更新** —— Windows 上一場玩完幾分鐘內數字自己更新。定時更新的
  間隔可以選，也可以關掉
...
**macOS 只在一台 Mac 上試過。** `.dmg` 沒有經過 Apple 公證，第一次打開會被
擋下來。到**系統設定 → 隱私權與安全性**，捲到下面，在
GFNUsage 旁邊按**強制打開**。
