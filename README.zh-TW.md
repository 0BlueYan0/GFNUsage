...
> **狀態：里程碑 3 完成，Windows 上的第一版功能到齊。**
> 可以直接用 NVIDIA 帳號登入（localhost 迴圈 OAuth），也可以從已安裝
> GeForce NOW 的機器匯入憑證；之後自行刷新，在系統匣顯示剩餘時數、
> 配速門檻、超支預測，以及在面板裡設定的不可遊玩時段。
> 設定匯出入、快照歷史、憑證到期提醒也都在這一輪完成。
>
> **macOS 尚未實測。** 登入流程正是為了讓沒裝 GFN 客戶端的 Mac 也能取得
> 憑證而做的，但開發機是 Windows，還沒有人在 Mac 上跑過。
...
- ✅ **用 NVIDIA 帳號登入** —— localhost 迴圈 OAuth，沒裝 GeForce NOW
  客戶端的機器也能自己取得憑證
- ✅ **設定匯出／匯入** —— 一份不可遊玩時段以 JSON 檔在多台機器之間搬移
- ✅ **憑證到期提醒** —— 憑證效期 90 天，到期前一週面板會先講一聲
